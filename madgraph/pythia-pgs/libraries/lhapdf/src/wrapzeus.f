...
*
c===========================================================================
      entry ZEUSinit(Eorder,Q2fit)
      print *,name
      if(name.eq.'QCDNUM_ZEUS_TR') then
         HEAVY = .FALSE.
	 VFN = .TRUE.
      else if(name.eq.'QCDNUM_ZEUS_FF') then
         HEAVY = .TRUE.
	 VFN = .FALSE.
      else if(name.eq.'QCDNUM_ZEUS_ZM') then
         HEAVY = .FALSE.
	 VFN = .FALSE.
      else
...
c      call grgive(nxgri,xxmin,xxmax,nqgri,qqmin,qqmax)
      ...
      DO IX = 1,NXGRI
        xX = XFROMIX(IX)
        UPVAL=UN*FF(Xx,UA,UB,UE,UC)
	

        DNVAL=DN*FF(Xx,DA,DB,DE,DC)


        SEA=SN*FF(Xx,SA,SB,SE,SC)

C        GN=(1-UN*AREA(UA,UB,UE,UC)-
C     .        DN*AREA(DA,DB,DE,DC)-
C     .        SN*AREA(SA,SB,SE,SC))/AREA(GA,GB,GE,GC)
        GN=XVAL(10)
        GLUON=GN*FF(Xx,GA,GB,GE,GC)
	


c        UMD=UBDBN*FF(X,DLA,DLB,DLE,DLC)
        UMD=DLN*FF(Xx,DLA,DLB,DLE,DLC)
...
c      ENDIF
c
c      AR1=(DGAMMA(A+1)*DGAMMA(B+1))/DGAMMA(A+B+2)
c      AR2=E*(DGAMMA(A+1.5)*DGAMMA(B+1))/DGAMMA(A+B+2.5)
c      AR3=C*(DGAMMA(A+2)*DGAMMA(B+1))/DGAMMA(A+B+3)
c      AREA=AR1+AR2+AR3
c     
