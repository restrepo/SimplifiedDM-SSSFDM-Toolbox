...
*   Check for Z3 breaking terms
     ...
     . XISINP.NE.0d0 .OR. M3HINP.NE.0d0) .AND. PFLAG.NE.0)THEN
...
*********************************************************************      
     ...
     . MQGUT,MUGUT,MDGUT,ML3GUT,ME3GUT,MLGUT,MEGUT
      ...
      WRITE(17,900) 2,"4.4.0      # Version number"
...
c      IF(OMGFLAG.NE.0)THEN
c	WRITE(17,899) "# " 
c	IF(OMGFLAG.GT.0)WRITE(17,911)
c     .   "# Omega h^2 (allowed:",OMGMIN," < Omega h^2 <",OMGMAX,"):"
c	IF(OMGFLAG.LT.0)WRITE(17,911)
c     .   "# Omega h^2 (allowed: Omega h^2 <",OMGMAX,"):"
c	IF(OMG.EQ.0d0)THEN
c	  WRITE(17,899) "# Cannot compute Omega h^2 (mLSP < 1 GeV)"
c	ELSEIF(OMG.EQ.-1d0)THEN
c	  WRITE(17,899) 
c     .      "# Lightest neutralino is not the LSP in micrOMEGAs"
c	ELSEIF(OMG.EQ.-2d0)THEN
c	  WRITE(17,899) "# Problem in micrOMEGAs"
c	ELSE
c	  WRITE(17,901) 10,OMG,"Omega h^2"
c	  omg_=printChannels(Xf,1.D-3,1.D-4,1,17)
c	ENDIF
c      ENDIF
c      IF(IABS(OMGFLAG).EQ.2 .OR. IABS(OMGFLAG).EQ.4)THEN
c	WRITE(17,907)"# sigma(p)_SI (allowed: sigma_p^SI < ",
cc     .   LUX(DABS(MNEU(1))),"):"
c	WRITE(17,901) 20,CSPSI,"sigma_p^SI"
c	WRITE(17,915)"# values used for sigma_piN,sigma_S",
c     .  " (strange content of the proton)"
c	WRITE(17,901) 30,sigmapiN,"sigma_piN"
c	WRITE(17,901) 40,sigmaS,"sigma_S"
c      ENDIF
 ...
 1    WRITE(18,899) "# "
      ...
      WRITE(18,900) 2,"4.4.0      # Version number"
 ...
 2    Continue 
c      IF(OMGFLAG.EQ.0) RETURN
c      WRITE(19,899) "# RELIC DENSITY CALCULATED BY MICROMEGAS"
c      WRITE(19,899) "#"
c      WRITE(19,899) "BLOCK RDINFO   # Program information"
c      WRITE(19,900) 1,"MicrOmegas # Dark matter package"
c      WRITE(19,900) 2,"4.4.0      # Version number"
c      IF(PROB(30).GT.0d0)WRITE(19,900) 3,
c     . "# Relic density too large (Planck)"
c      IF(PROB(30).LT.0d0.AND.PROB(30).NE.-1d0.AND.PROB(30).NE.-2d0)
c     . WRITE(19,900) 3,"# Relic density too small (Planck)"
c      IF(PROB(31).NE.0d0)WRITE(19,900) 3,
c     . "# Excluded by LUX"
c      IF(IFAIL.EQ.0.OR.IFAIL.GE.10)THEN
c        IF(OMGFLAG.NE.0)CALL printRelDen(PROB,19) 
c      ELSE
c        WRITE(19,900) 4,"# Cannot compute Omega h^2 (0<IFAIL<10)"
c      ENDIF
