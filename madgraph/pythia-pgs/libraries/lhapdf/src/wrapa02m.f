      ...
      integer nxb,nq,np,nvar,pdfset,iset
     ...
     +      ,dsp(0:nvar,nxb,nq+1,0:np)
      ...
      save f,npdf,npar,pdfset,dels,delx,x1,delx1,xlog1,nxbb,xx
     +,fsp,bsp,csp,dsp
c      save 
...
*
      entry A02Mpdf(iset)
      pdfset=iset
      if ((pdfset.lt.0).or.(pdfset.gt.npar)) then
         write(*,*) 'A02M PDF set:'
         write(*,*) 'PDF member out of range:'
         write(*,*) 'member = ',pdfset,'    member range = (0,',npar,')'
         stop
      endif
      return
      end
