...
Subroutine FeThreeBodyDecay(n_in,MAh,MAh2,MChi,MChi2,MetI,MetI2,Metp,Metp2,           & 
...
& lam3,lam5,Yn,Yu,Yd,Ye,Mn,mH2,mEt2,v,gTetp,gThh,gTVWp,gTVZ,gFeChiFeChi,gFeFecFdFd,      & 
...
Complex(dp) :: cplcFdFdhhL(3,3),cplcFdFdhhR(3,3),cplcFdFdVZL(3,3),cplcFdFdVZR(3,3),cplcFeChicetpL(3,3),& 
...
Real(dp),Intent(inout) :: gFeChiFeChi(3,3,3,3),gFeFecFdFd(3,3,3,3),gFeFecFeFe(3,3,3,3),gFeFecFuFu(3,3,3,3),     & 
& gFeFeFvFv(3,3,3,3),gFeFvcFuFd(3,3,3,3)

Real(dp),Intent(in) :: gTetp,gThh,gTVWp,gTVZ

Real(dp) :: gFeChiFeChii(3,3,3),gFeFecFdFdi(3,3,3),gFeFecFeFei(3,3,3),gFeFecFuFui(3,3,3),         & 
& gFeFeFvFvi(3,3,3),gFeFvcFuFdi(3,3,3)

Real(dp) :: gTetptemp,gThhtemp,gTVWptemp,gTVZtemp
...
& MHp,MHp2,MVWp,MVWp2,MVZ,MVZ2,TW,ZDR,ZER,ZUR,ZDL,ZEL,ZUL,UV,ZW,ZX,ZZ,g1,g2,             & 
& g3,lam1,lam2,lam4,lam3,lam5,Yn,Yu,Yd,Ye,Mn,mH2,mEt2,v,cplcFdFdhhL,cplcFdFdhhR,         & 
& cplcFdFdVZL,cplcFdFdVZR,cplcFeChicetpL,cplcFeChicetpR,cplcFeFehhL,cplcFeFehhR,         & 
& cplcFeFeVZL,cplcFeFeVZR,cplcFeFvcVWpL,cplcFeFvcVWpR,cplcFuFdVWpL,cplcFuFdVWpR,         & 
& cplcFuFuhhL,cplcFuFuhhR,cplcFuFuVZL,cplcFuFuVZR,cplChiFeetpL,cplChiFeetpR,             & 
& cplFvFeVWpL,cplFvFeVWpR,cplFvFvVZL,cplFvFvVZR,deltaM)
...
gFeChiFeChii = 0._dp 
Call FeToChiFeChi(i_run,MChi,MFe,Metp,cplcFeChicetpL,cplcFeChicetpR,cplChiFeetpL,     & 
& cplChiFeetpR,IntegralSs,IntegralSSss,IntegralSSst,NSs,NSSss,NSSst,gTetptemp,           & 
& deltaM,epsI,check,gFeChiFeChii)

gFeChiFeChi(i_run,:,:,:) = gFeChiFeChii 
gT(i_run) = gT(i_run) + Sum(gFeChiFeChii) 
...
Do gt1=1,3
  Do gt2=1,3
    Do gt3=gt1,3
gPartial(i1,n_length)= gFeChiFeChi(i1,gt1,gt2,gt3)
n_length=n_length+1
     End Do 
  End Do 
End Do 
...
Subroutine FeToChiFeChi(iIN,MChi,MFe,Metp,cplcFeChicetpL,cplcFeChicetpR,              & 
& cplChiFeetpL,cplChiFeetpR,IntegralSs,IntegralSSss,IntegralSSst,NSs,NSSss,              & 
& NSSst,gTetp,deltaM,epsI,check,g,WriteContributions)

Implicit None 
 
Real(dp),Intent(in) :: MChi(3),MFe(3),Metp

Complex(dp),Intent(in) :: cplcFeChicetpL(3,3),cplcFeChicetpR(3,3),cplChiFeetpL(3,3),cplChiFeetpR(3,3)
...
coup(2) = Conjg(cplcFeChicetpL(iIN,gt1)) 
coup(1) = Conjg(cplcFeChicetpR(iIN,gt1)) 
coup(4) = Conjg(cplChiFeetpL(gt3,gt2)) 
coup(3) = Conjg(cplChiFeetpR(gt3,gt2))
...
coup(2) = Conjg(cplcFeChicetpL(iIN,gt3)) 
coup(1) = Conjg(cplcFeChicetpR(iIN,gt3)) 
coup(4) = Conjg(cplChiFeetpL(gt1,gt2)) 
coup(3) = Conjg(cplChiFeetpR(gt1,gt2))
Call IntegrateScalarSS(Boson2,mass,coup,deltaM,epsI,IntegralSs,NSs,resR, check) 
If (gt1.Eq.gt3) Then 
resR=resR/2._dp 
End If
...
coup2(1) = cplcFeChicetpL(iIN,gt3) 
coup2(2) = cplcFeChicetpR(iIN,gt3) 
coup2(3) = Conjg(cplcFeChicetpL(iIN,gt1)) 
coup2(4) = Conjg(cplcFeChicetpR(iIN,gt1))  
coup2(5) = cplChiFeetpL(gt1,gt2) 
coup2(6) = cplChiFeetpR(gt1,gt2) 
coup2(7) = Conjg(cplChiFeetpL(gt3,gt2)) 
coup2(8) = Conjg(cplChiFeetpR(gt3,gt2)) 
...
Write(*,*) "Fe->Chi Fe Chi Propagator: conj[etp]" 
Write(*,*)  "M_in: ",m_in 
Write(*,*)  "mass: ",mass 
Write(*,*)  "coup: ",coup 
...
End Module Fe3Decays_Scotogenic 
