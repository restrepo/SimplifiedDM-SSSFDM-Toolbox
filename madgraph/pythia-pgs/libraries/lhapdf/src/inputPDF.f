      ...
      include 'parmsetup.f'
      character*16 name
      integer nmem,ndef,mem
      common/NAME/name,nmem,ndef,mem		!*** don't need any of these (jcp) ***
      real*4 xp(40),x4,fgdis4
      character*16 s1,s2
      integer i,j,iset,nop,parmN,Nfunc,Fw,nfmax,M
      real*8 x,N,b0,Poly,pdf(-6:6),Fparm(nopmax),F(nofmax)
      real*8 Ccoef(-6:6,nofmax),Fpow(nofmax)
      real*8 Q,Treshold(-6:6)
      data Treshold/13*0d0/
...
c - to add the mrst2004 gluon convolution
         if (Ftype(i).eq.201) then
	   xp(2) = Fparm(Fmap(i,1))
	   xp(3) = Fparm(Fmap(i,2))
	   xp(23)= Fparm(Fmap(i,3))
	   xp(16)= Fparm(Fmap(i,4))
	   xp(5) = Fparm(Fmap(i,5))
	   xp(40)= Fparm(Fmap(i,6))
	   xp(24)= Fparm(Fmap(i,7)) 
	   xp(20)= Fparm(Fmap(i,8)) 
           xp(4) = Fparm(Fmap(i,9)) 
	   xp(1) = Fparm(Fmap(i,10))
	   x4 = x
...
c		  print *,i,j,Ccoef(i,j),F(j)
            ...
            if (Ctype(i).eq.101) then
               if (i.eq.-2) then
                  pdf(i)=F(int(Ccoef(i,1)))/(F(int(Ccoef(i,2)))+1d0)
               endif
               if (i.eq.-1) then
                  pdf(i)=F(int(Ccoef(i,1)))/(1d0/F(int(Ccoef(i,2)))+1d0)
               endif
               if (i.eq.1) then
                  pdf(i)=F(1)+pdf(-1)
               endif
...
*
      ...
      IF(IER.NE.0) write(16,*) 'GAMMA ERROR: IER= ',IER,X1,X2
...
c      AL = 0.550/(4.* 3.14159)
      ...
      rx=sqrt(x)
      FF1=BETA(XP(2),XP(3)+1.)+XP(16)*BETA(XP(2)+1.,XP(3)+1.)+
     XXP(23)*BETA(XP(2)+0.5,XP(3)+1.)
      FF2=BETA(XP(2)+1.,XP(3)+1.)+XP(16)*BETA(XP(2)+2.,XP(3)+1.)+
     XXP(23)*BETA(XP(2)+1.5,XP(3)+1.)
      FF3=BETA(XP(5),ETA4+1.)+XP(20)*BETA(XP(5)+1.,ETA4+1.)+
     XXP(24)*BETA(XP(5)+0.5,ETA4+1.)
      FF4=BETA(XP(5)+1.,ETA4+1.)+XP(20)*BETA(XP(5)+2.,ETA4+1.)+
     XXP(24)*BETA(XP(5)+1.5,ETA4+1.)
      COEFU=2.*XP(4)/FF1
      COEFD=   XP(4)/FF3
c	   print *,'coefu ',coefu
c	   print *,'coefd ',coefd
