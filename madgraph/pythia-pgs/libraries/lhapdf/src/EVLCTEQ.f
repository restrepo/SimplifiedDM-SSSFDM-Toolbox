      ...
      COMMON / LhCtQCDPAR / AL, NF, NORDER, SET
      LOGICAL SET
      ...
      IF (IRT .EQ. 1) THEN
         CALL CtLhWARNR (IW1, 'AMU < ALAM in CtLhALPI', 'MU', AMU,
     >              ALM, BIG, 1)
  ...
  101         FORMAT (/ ' Initiation parameters:   Qini, Ipd0, Ihdn ' /
   ...
   12 IRET = 2
      ...
      COMMON / LhCtEVLPAC / AL, IKNL, IPD0, IHDN, NfMx
      COMMON / LhCtPEVLDT / UPD(MXPQX), KF, Nelmt
  ...
  332   CONTINUE
   31 CONTINUE
    ...
    5     DO 333 IFLV = 1, NEFF
     ...
     >     NEFF, UPD(JI( IFLV)+2), UPD(JI( IFLV)+1))
           IF (IFLV .LE. MXVAL)
...
333      CONTINUE
  ...
  335   CONTINUE
        CALL CtLhHQRK (NX, TT, NEFF+1, UPD(JI(0)+2), UPD(JI(NEFF+1)+2))
        DO 32 IZ = 1, NX
...
110	continue
      ...
      Do it = 1, nqvec
       ...
       ElseIf  (JLx .Eq. Nx-1) Then
        Call CtLhPolint4 (XVpow(Nx-3), Upd(J1), 4, ss, Fx, Dfx)
        Fvec(it) = Fx
       Else 
         sf2 = Upd(J1+1)
         sf3 = Upd(J1+2)
         Fvec(it) = (const5*(Upd(J1) 
