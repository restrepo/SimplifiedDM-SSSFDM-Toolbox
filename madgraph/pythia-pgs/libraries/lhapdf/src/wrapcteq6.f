      ...
      Common
     > / CtqPar1 / Al, XV(0:MXX), TV(0:MXQ), UPD(0:nhess,MXPQX)
     > / CtqPar2 / Nx, Nt, NfMx
...
*
      ...
      read(1,*)nmem,ndef	!*** nmem+1=number of members; ndef is not used for anything ***
      if(nmem .gt. nhess) then
         print *,'fatal error:  nmem=',nmem,' > nhess=',nhess
         stop
      endif
      Read  (1, '(A)') Line
      Read  (1, '(A)') Line
...
c      print *,'NX=',NX,' NT=',NT,' NfMx=',NfMx,' Npts=',Npts	!*** temporary ***

c **  do ihess = 0,nhess	!*** old version ***
      do ihess = 0,nmem		!*** new version: allows nmem < nhess ***
